...
2. Написание [приложения](pg-deployer/README.md) по ТЗ (далее `pg-loader`), которое будет загружено на Ansible Control Node.
3. Написание входной точки - задача которой инициализировать сборку инфраструктуры, дождаться ее завершения поключиться к Ansible Control Node и запустить `pg-loader` передав входными параметрами имена серверов. 
...
Для управления конфигурациями я решил использовать Ansible, поэтому ввод имен серверов в приложение не требуется, список серверов можно изменять в `inventory.ini`